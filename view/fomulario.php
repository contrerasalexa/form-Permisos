...
$query_municipios = "SELECT id_municipio_pk, nombre FROM municipio WHERE estatus = 1";
$stmt_municipios = $cnn->prepare($query_municipios);
$stmt_municipios->execute();
$municipios = $stmt_municipios->fetchAll(PDO::FETCH_ASSOC);

// Consultar permisos
$query_permisos = "SELECT id_permiso_pk, nombre FROM permisos WHERE estatus = 1";
$stmt_permisos = $cnn->prepare($query_permisos);
$stmt_permisos->execute();
$permisos = $stmt_permisos->fetchAll(PDO::FETCH_ASSOC);

// Consultar unidades
$query_unidades = "SELECT id_unidades_pk, nombre FROM unidades WHERE estatus = 1";
